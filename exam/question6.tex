...
Вначале идут $i$ элементов без повтора, потом идёт цикл $j-i$
\end{dfn}
...
\begin{proof}
Пусть $j-i=m$, проверим элемент $a^{i+(m-i)\Mod m}$
$$(a^{i+(m-i)\Mod m})^2=a^{2i+2(m-i)\Mod m}=a^{i+(2i+2(m-i))\Mod m-i)\Mod m}$$
\begin{align*}
i+(2i+2(m-i)-i)\Mod m-i)\Mod m&=i+(2i+2m-2i-i)\Mod m\\
&=i+(2m-i)\Mod m\\
&=i+(m-i)\Mod m
\end{align*}
Следовательно $a^{i+(m-i)\Mod m}$ - идемпотент
\end{proof}
\end{thm}
\end{document}